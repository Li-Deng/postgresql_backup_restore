                ...
                if (!options.host || !options.user || !options.port || !options.dumpPath || !options.database) {
                    ...
                    fs.exists((extName === '') ? options.dumpPath : dirName, function (exists) {
                        if (!exists) {
                            ...
                            if (options.password) {
                                command = util.format('PGPASSWORD="%s" pg_dump -i -h %s -p %d -U %s -F c -b -v -c -f %s %s', options.password, options.host, options.port, options.user, filePath, options.database);
                            } else {
                                command = util.format('pg_dump -i -h %s -p %d --no-password -U %s -F c -b -v -c -f %s %s', options.host, options.port, options.user, filePath, options.database);
                            }
                ...
                if (!options.user || !options.host || !options.port || !options.sqlFilePath || !options.database) {
                    ...
                    fs.exists(options.sqlFilePath, function (exists) {
                        if (!exists) {
                            ...
                            if (options.password) {
                                command = util.format('PGPASSWORD="%s" psql -h %s -p %d -U %s -c --if-exists -f %s %s', options.password, options.host, options.port, options.user, options.sqlFilePath, options.database);
                            } else {
                                command = util.format('psql -h %s -p %d --no-password -U %s -c --if-exists -f %s %s', options.host, options.port, options.user, options.sqlFilePath, options.database);
                            }
                            ls = exec(command, function (error, stdout, stderr) {
                                if (error !== null) {
                                    callback(error, null, null);
